...
[
    dnl save current state of AC_PROG_CC_C11
    FP_COPY_SHELLVAR([CC],[fp_save_CC])
    FP_COPY_SHELLVAR([CFLAGS],[fp_save_CFLAGS])
    FP_COPY_SHELLVAR([CPPFLAGS],[fp_save_CPPFLAGS])
    FP_COPY_SHELLVAR([ac_cv_prog_cc_c11],[fp_save_cc_c11])
    dnl set local state
    CC="$$1"
    CFLAGS="$$2"
    CPPFLAGS="$$3"
    unset ac_cv_prog_cc_c11
    dnl perform detection
    AC_PROG_CC_C11
    fp_cc_c11="$ac_cv_prog_cc_c11"
    case "x$ac_cv_prog_cc_c11" in
      x)   ;; # noop
      xno) AC_MSG_ERROR([C11-compatible compiler needed]) ;;
      *)   $2="$$2 $ac_cv_prog_cc_c11"
           $3="$$3 $ac_cv_prog_cc_c11"
           ;;
    esac
    ...
    FP_COPY_SHELLVAR([fp_save_cc_c11],[ac_cv_prog_cc_c11])
    dnl cleanup
    unset fp_save_CC
    unset fp_save_CFLAGS
    unset fp_save_cc_c11
])
